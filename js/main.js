...
const deleteGoods = document.querySelector('.delete-goods');
let totalCount;
...
const cart = {
	...
	minusGood(id){
		for (const item of this.cartGoods) {
			if (item.id === id) { // прибавляем товар, если совпали id
				if (item.count <= 1) { // удаляем товар, если он один или меньше
					this.deleteGood(id);
				} else {
					item.count--;
					totalCount--;
	...
	deleteAllGods(){
		cartGoods: []; // очищаем массив. не получается
		cartTableGoods.textContent = ''; // очищаем строки с товарами из корзины
		cardTableTotal.textContent = ''; // очищаем тотал прайс
		cartCount.textContent = ''; //очищаем кол-во всех товаров на кнопке cart 
	}
};

//* очищаем корзину по кнопке
deleteGoods.addEventListener('click', cart.deleteAllGods);
